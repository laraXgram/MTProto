...
foreach ($methodsByNs as $ns => $nsMethods) {
    ...
    foreach ($nsMethods as $m) {
        ...
        // Build parameter list (skip flags fields, skip random_id — auto-generated)
        ...
        foreach ($m->getParams() as $p) {
            ...
            // Skip random_id — auto-generated by preprocessor
            if ($pName === 'random_id' && ($pType === 'long' || $p->isVector())) {
                continue;
            }
            ...
            // Complex TL types → accept array|int|string for peers, array for rest
            $docHint = $docType;
...
$buf .= " */\n\n";
$buf .= "declare(strict_types=1);\n\n";
$buf .= "namespace LaraGram\\MTProto\\Core;\n\n";
$buf .= "/**\n";
...
foreach ($allMethodsFlat as $fullTL => $info) {
    ...
    // Build short param signature for @method
    $paramParts = [];
    foreach ($m->getParams() as $p) {
        if ($p->getType() === '#') continue;
        if ($p->getName() === 'random_id') continue;

        $pName = $p->getName();
        $docType = tlTypeToDocType($p->getType(), $p->isVector());
        if ($p->getType() === 'true') $docType = 'bool';
        if (in_array($p->getType(), ['InputPeer', 'InputUser', 'InputChannel'], true)) {
            $docType = 'array|int|string';
        }

        if ($p->isOptional()) {
            $default = ($p->getType() === 'true') ? 'false' : 'null';
            $paramParts[] = "{$docType}|null \${$pName} = {$default}";
        } else {
            $paramParts[] = "{$docType} \${$pName}";
        }
    }
